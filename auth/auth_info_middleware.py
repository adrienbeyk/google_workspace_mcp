                        ...
                        if auth_provider:
                            try:
                                ...
                                if verified_auth:
                                    ...
                                    # === VIVENTIUM START ===
                                    # Feature: Persist Google local auth state after successful bearer verification.
                                    # Purpose: The MCP can prove auth from a live access token even when the
                                    # durable local refresh-token/credential files have not been materialized yet.
                                    # Flush both local stores here so a restart on a new machine remains connected.
                                    # === VIVENTIUM END ===
                                    if credentials and user_email and getattr(credentials, "refresh_token", None):
                                        try:
                                            get_credential_store().store_credential(user_email, credentials)
                                        except Exception as store_exc:
                                            logger.warning(
                                                "Failed to persist Google credential store for %s: %s",
                                                user_email,
                                                store_exc,
                                            )
                                    if hasattr(auth_provider, "persist_refresh_token_for_access_token"):
                                        try:
                                            auth_provider.persist_refresh_token_for_access_token(token_str)
                                        except Exception as persist_exc:
                                            logger.warning(
                                                "Failed to persist Google refresh-token state for %s: %s",
                                                user_email,
                                                persist_exc,
                                            )
